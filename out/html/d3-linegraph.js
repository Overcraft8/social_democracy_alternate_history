...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['ccf', 'kpd', 'ddp', 'z', 'dvp', 'dnvp', 'nsdap', 'other'];
    }
    if (!partyColors) {
        partyColors = {'ccf': '#E3000F', 'kpd': '#8B0000', 'ddp': '#DCCA4A', 'cpc': '#000', 'dvp': '#D5AC27', 'dnvp': '#3f7bc1', 'nsdap': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'ccf': 'CCF', 'kpd': 'KPD', 'ddp': 'DDP', 'cpc': 'CPC + BVP', 'dvp': 'DVP', 'dnvp': 'DNVP', 'nsdap': 'NSDAP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
